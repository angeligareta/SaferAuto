...
QImage draw_boxes(cv::Mat mat_img, std::vector<bbox_t> result_vec, std::vector<std::string> obj_names, unsigned int wait_msec = 0) {
...
void process_video(std::string cfg_file, std::string names_file, std::string weights_file, std::string filename, MainWindow WIN)
{
    ...
    try {
        ...
        if (file_ext == "avi" || file_ext == "mp4" || file_ext == "mjpg" || file_ext == "mov") {	// video file
            ...
            for(cv::VideoCapture cap(filename); cap >> frame, cap.isOpened();) {
                ...
                QImage imdisplay = draw_boxes(frame, result_vec, obj_names, time_bt_detections);
